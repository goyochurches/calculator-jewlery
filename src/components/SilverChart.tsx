...
interface SilverChartProps {
  data: ChartDataPoint[]
  changePercent: number
}

export function SilverChart({ data, changePercent }: SilverChartProps) {
  ...
  return (
    <Card className="rounded-[28px] border border-white/80 bg-white/92 shadow-[0_18px_45px_rgba(15,23,42,0.08)]">
      <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 pb-4">
        <div>
          <CardTitle className="text-base font-semibold text-slate-900">Silver (XAG)</CardTitle>
        ...
        <ResponsiveContainer width="100%" height={220}>
          <LineChart data={data}>
            ...
            <YAxis
              ...
              width={60}
            />
            <Tooltip formatter={(value) => [`$${Number(value ?? 0).toFixed(2)}`, 'Silver']} />
            <Line
              type="monotone"
              dataKey="silver"
              stroke="#64748b"
