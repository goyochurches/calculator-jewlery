...
import { ArrowLeft, Copy, ImageOff, Trash2 } from 'lucide-react'
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="space-y-4 lg:col-span-2">
          ...
          <Card>
            ...
            {metalRows.length === 0 && <p className="text-sm text-slate-400">No metal rows.</p>}
            {metalRows.map((r, i) => (
              <LineItem key={i} label={JEWELRY_METAL_OPTIONS[r.metalKey]?.label ?? r.metalKey} value={`${r.weightGrams}g`} />
            ))}
          </Card>
                ...
                <div key={i} className="border-b border-slate-50 py-2 last:border-0">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-sm font-semibold text-slate-800">
                      {s.carats ? `${s.carats}ct` : ''} {s.contribution != null ? `· $${s.contribution.toLocaleString('en-US', { minimumFractionDigits: 2 })}` : ''}
                    </span>
                  </div>
                  {s.comments && <p className="mt-1 text-xs text-slate-400">{s.comments}</p>}
                </div>
              ))}
            </Card>
          )}
