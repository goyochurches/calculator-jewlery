        ...
        <CardHeader className="flex flex-row items-center justify-between gap-3 border-b border-slate-100">
          <div>
            ...
            <p className="text-sm text-slate-500">
              {quotes.length === 0 ? 'No quotes yet.' : `${quotes.length} quote${quotes.length === 1 ? '' : 's'} on file.`}
            </p>
          </div>
          <Button
          ...
          {quotes.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[640px] text-sm">
                ...
                <tbody>
                  {quotes.map(q => (
                    <tr key={q.id} className="border-b border-slate-100 transition-colors last:border-0 hover:bg-slate-50/80">
