...
export function QuoteBuilderPage() {
  ...
  interface StoneRow {
     ...
     *  can't be collapsed/saved on the default value alone. See
     *  `stoneTypeChosen` gating in renderStoneRow and handleQuoteReady. */
    ...
    // stone's cost. Setting labor is unaffected. For a custom (no-preset)
    // size this is DERIVED — kept in sync from manualPricePerCarat × carats
    // rather than typed directly; see onStoneManualPricePerCaratChange.
    manualPrice: string
    /** Cost per carat the jeweler types by hand for a custom (no-preset)
     *  size — the system multiplies by carats to get manualPrice/the total.
     *  Only used/shown when sizeKey === '' (no size/cut match in the system). */
  ...
  const [emkayTotal, setEmkayTotal] = useState(0)
      ...
      <div key={stone.uid} className={`relative rounded-2xl border ${theme.ring} ${theme.tint} p-4 space-y-3 overflow-hidden shadow-sm transition hover:shadow-md`}>
        ...
        <div className="grid gap-3 md:grid-cols-2 pl-2">
          ...
          {/* Grouped so it's unmistakable these two overrides are related
              ...
              Size so picking a price is the very next thing after picking
              what stone this is — not buried below Carats/Setting/Color. */}
              ...
              <div className="space-y-1">
                ...
                {caratsNum > 0 && (
                  <p className="text-[11px] text-slate-500">
                    = ${stoneCost.toLocaleString('en-US', { minimumFractionDigits: 2 })} total ({caratsNum} ct × $
        ...
        <div className="space-y-4">
          ...
          <Card id="tour-qb-material" className="rounded-[30px] border border-white/80 bg-white/92 shadow-[0_20px_60px_rgba(15,23,42,0.08)]">
            ...
            <CardContent className="grid gap-5 pt-6 md:grid-cols-2">
              {!rnMode && (<>
              {metalRows.length === 1 ? (<>
              {/* Single-metal: classic two-field layout, identical to before */}
              <div className="space-y-2">
                <label className="text-sm font-semibold text-slate-900">Metal</label>
                <select
                  value={metalRows[0].metal}
                  onChange={e => setMetalRows([{ ...metalRows[0], metal: e.target.value as JewelryMetalOption }])}
                  className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-slate-400 focus:bg-white"
                >
                  {METAL_GROUPS.map(g => (
                    <optgroup key={g.group} label={g.group}>
          ...
          <Card id="tour-qb-stones" className="relative overflow-hidden rounded-[30px] border border-white/80 bg-white/92 shadow-[0_20px_60px_rgba(15,23,42,0.08)]">
            ...
            <CardContent className="space-y-4 pt-6">
              ...
              <div className="group/section relative overflow-hidden rounded-2xl border border-amber-100 bg-white p-4 shadow-sm transition hover:shadow-md">
                  ...
                  <div className="mt-3 pl-2 space-y-3">
                    {emkayStones.map((es, idx) => (
                      <div key={es.uid} className={`relative overflow-hidden rounded-2xl border ${emkayTheme.ring} ${emkayTheme.tint} p-4 shadow-sm transition hover:shadow-md`}>
                        <span className={`absolute left-0 top-0 bottom-0 w-1.5 ${emkayTheme.bar}`} aria-hidden />
                        <div className="flex gap-3 pl-2">
                          ...
                          <div className="min-w-0 flex-1">
                            <div className="mb-1 flex items-center justify-between gap-2">
                              ...
                              <button type="button" onClick={() => removeEmkayStone(es.uid)}
                                aria-label="Remove EMKAY stone"
                                className="flex h-7 w-7 items-center justify-center rounded-full bg-white/70 text-slate-400 transition hover:bg-rose-50 hover:text-rose-600">
                                <X className="h-4 w-4" />
                              </button>
                            </div>
                ...
                {emkayOpen && (
                  <div className="mt-4 space-y-3 border-t border-amber-100 pt-4 pl-2">
                      ...
                      <>
                        ...
                        {emkayTotalPages > 1 && (
                          <div className="flex items-center justify-between gap-3 rounded-2xl bg-slate-50 px-4 py-3">
                            <span className="text-xs text-slate-500">{emkayTotal} stones total</span>
                            <div className="flex items-center gap-3">
                              <button type="button" disabled={emkayPage <= 0} onClick={() => setEmkayPage(p => Math.max(0, p - 1))}
                              ...
                              <span className="text-sm font-semibold text-slate-700">Page {emkayPage + 1} / {emkayTotalPages}</span>
                              <button type="button" disabled={emkayPage >= emkayTotalPages - 1} onClick={() => setEmkayPage(p => Math.min(emkayTotalPages - 1, p + 1))}
                                className="inline-flex items-center gap-1 rounded-full border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 shadow-sm transition hover:border-amber-400 hover:text-amber-700 disabled:opacity-40 disabled:hover:border-slate-300 disabled:hover:text-slate-700">
          ...
          <Card className="rounded-[30px] border border-white/80 bg-white/92 shadow-[0_20px_60px_rgba(15,23,42,0.08)]">
            <CardContent className="p-6">
              ...
              {/* ── Every stone's own contribution, so it's obvious what
                  makes up the total above. ─────────────────────────────── */}
              {(mainStones.length + sideStones.length + meleeStones.length + customerStones.length + emkayStones.length) > 0 && (
                <div className="mt-4 space-y-1.5 border-t border-slate-100 pt-4 text-sm">
                  ...
                  {customerStones.map((cs, i) => {
                    const qty = Math.max(1, parseNum(cs.quantity || '1') || 1)
                    const feeOverride = cs.setterFeeOverride.trim()
                    const fee = feeOverride !== '' ? parseNum(feeOverride) : (config.setterMap[cs.setterType]?.fee ?? 0)
                    const contribution = qty * fee
                    const gem = gemstones.find(g => g.id === cs.gemstoneId)
                    return (
                      <div key={cs.uid} className="flex items-center justify-between gap-3">
                        <span className="flex min-w-0 items-center gap-2 text-slate-600">
                          <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-rose-500" aria-hidden />
                          <span className="shrink-0 font-semibold text-slate-700">Customer #{i + 1}</span>
                          <span className="truncate text-slate-400">{gem?.name ?? 'Customer stone'} (setting only)</span>
                        </span>
                        <strong className="shrink-0 tabular-nums text-slate-900">${contribution.toLocaleString('en-US', { minimumFractionDigits: 2 })}</strong>
                      </div>
                    )
                  ...
                  {emkayStones.map((es, i) => {
                    const qty = Math.max(1, parseNum(es.quantity || '1') || 1)
                    const feeOverride = es.setterFeeOverride.trim()
                    const fee = feeOverride !== '' ? parseNum(feeOverride) : (config.setterMap[es.setterType]?.fee ?? 0)
                    const contribution = qty * es.priceUsd + qty * fee
                    return (
                  ...
                  <div className="flex items-center justify-between gap-3 border-t border-slate-100 pt-2 text-sm font-bold text-slate-900">
                    <span>Total</span>
                    <span>${(pricing.diamondCost + pricing.settingFee + pricing.customerSettingFee + pricing.emkayCost + pricing.emkaySettingFee).toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
                  </div>
                </div>
              )}
            </CardContent>
